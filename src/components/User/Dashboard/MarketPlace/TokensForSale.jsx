...
const TokensForSale = () => {
    ...
    useEffect(() => {
        const fetchTokens = async () => {
            try {
                ...
                toast.current.show({ severity: 'error', summary: 'Error', detail: 'Error fetching token details' });
            }
        };
        fetchTokens();
    }, []);
    ...
    const renderIcon = (rowData) => {
        return <img src={`http://localhost:5000/${rowData.assetIcon}`} className="border-round" alt={rowData.tokenName} style={{ width: '50px', height: '50px' }} />;
    };
    ...
    return (
        <Layout>
            <div>
                ...
                <DataTable value={tokens} selectionMode="single" onRowSelect={onRowSelect}>
